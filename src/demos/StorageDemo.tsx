...
export default function StorageDemo() {
  ...
  function handleSave() {
    const k = key.trim();
    if (!k) return;
    setStore((s) => ({ ...s, [k]: val.trim() || t('demoUI.storage.emptyValue') }));
    setKey('');
    setVal('');
  }
  ...
  return (
    <DemoPanel
      desc={t('demoText.storage.desc')}
      note={t('demoText.storage.note')}
    >
      <div className="demo-box">
        <div className="kv-row">
          <input type="text" placeholder={t('demoUI.storage.keyPlaceholder')} value={key} onChange={(e) => setKey(e.target.value)} />
          <input type="text" placeholder={t('demoUI.storage.valuePlaceholder')} value={val} onChange={(e) => setVal(e.target.value)} />
          <button className="btn btn-primary" style={{ padding: '9px 16px' }} onClick={handleSave}>{t('demoUI.storage.setButton')}</button>
        </div>
        <div className="kv-list">
          {entries.length === 0
            ? <div className="kv-item"><span>—</span><span>{t('demoUI.storage.noKeysYet')}</span></div>
            : entries.map(([k, v]) => (
